...
import numpy as np
from models import fc_block, make_net
...
class VAECOV1(nn.Module):
    def __init__(self, opts):
        ...
        A1 = torch.eye(self.z_dim).repeat(self.num_samples, self.num_samples) - torch.eye(self.z_dim * self.num_samples)
...
class Encoder(nn.Module):
    def __init__(self, x_dim, h_dim, z_dim, num_samples):
        ...
        self.fc3 = nn.Linear(h_dim, z_dim * (num_samples**2))

    def forward(self, x):
        z = self.net.forward(x)
        return self.fc1(z), self.fc2(z), self.fc3(z)
